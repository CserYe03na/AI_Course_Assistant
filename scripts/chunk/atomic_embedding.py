#!/usr/bin/env python3
"""Create embeddings for indexable atomic chunks."""
...
DEFAULT_INPUT_GLOB = "data/chunk/*_atomic_chunks.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-path",
        action="append",
        help="Explicit atomic chunk JSON path. Can be passed multiple times.",
    )
    parser.add_argument(
        "--input-glob",
...
def resolve_course_input_paths(course_ids: Sequence[str]) -> List[Path]:
    return [Path("data/chunk") / f"{course_id}_atomic_chunks.json" for course_id in course_ids]


def resolve_output_path(input_path: Path, output_dir: str | None) -> Path:
    if input_path.name.endswith("_atomic_chunks.json"):
        output_name = input_path.name.replace("_atomic_chunks.json", "_atomic_embeddings.json")
    else:
            ...
            vectors.append(
                {
                    ...
                    "document": {
                        ...
                        "raw_fields": chunk.get("raw_fields"),
                    },
                }
            )
    return vectors
...
def build_output_payload(
    *,
    ...
    return {
        "source_atomic_chunk_path": str(input_path.as_posix()),
        "course_id": atomic_chunk_payload.get("course_id"),
        "course_name": atomic_chunk_payload.get("course_name"),
        "source_type": atomic_chunk_payload.get("source_type"),
...
def main() -> None:
    ...
    if not input_paths:
        raise FileNotFoundError("No atomic chunk JSON files found")

    missing_paths = [path for path in input_paths if not path.exists()]
    if missing_paths:
        missing = ", ".join(str(path) for path in missing_paths)
        raise FileNotFoundError(f"Atomic chunk JSON file(s) not found: {missing}")
    ...
    for input_path in input_paths:
        ...
        candidates = [
            chunk for chunk in all_candidates if str(chunk.get("chunk_id")) not in existing_vectors_by_id
        ]
        print(
            f"Embedding {len(candidates)} new chunks from {input_path} "
        ...
        for chunk in all_candidates:
            ...
            for vector in new_vectors:
                if vector.get("id") == chunk_id:
                    merged_vectors.append(vector)
                    break
        output_payload = build_output_payload(
            input_path=input_path,
            atomic_chunk_payload=payload,
        ...
        print(
            f"Wrote {len(merged_vectors)} embeddings to {output_path}",
            flush=True,
        )
